...
let
	...
	stackOpts = { name, ... }: {
		options = {
			...
			stack = mkOption {
				description = "The stack configuration, as either a string containing a YAML/JSON value, or as a Nix attrset.";
				type = types.attrs;
			};
...
in {
	...
	in {
		...
		systemd.services = (mapAttrs' generateService cfg);
